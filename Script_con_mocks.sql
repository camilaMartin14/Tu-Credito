...
-- Script de Datos de Prueba (Seed Data) para TuCredito
-- Ejecutar en SQL Server

USE TuCreditoDB; -- Asegúrate de usar el nombre correcto de tu base de datos
GO

-- Deshabilitar constraints para limpiar tablas
EXEC sp_msforeachtable "ALTER TABLE ? NOCHECK CONSTRAINT all"
GO

-- Limpiar tablas (Orden inverso de dependencia)
DELETE FROM Pagos;
DELETE FROM Cuotas;
...
-- Reseed Identity (Reiniciar contadores de ID)
DBCC CHECKIDENT ('Pagos', RESEED, 0);
DBCC CHECKIDENT ('Cuotas', RESEED, 0);
DBCC CHECKIDENT ('Prestamos', RESEED, 0);
-- DBCC CHECKIDENT ('Prestatarios', RESEED, 0); -- DNI no es identity
DBCC CHECKIDENT ('Garantes', RESEED, 0);
DBCC CHECKIDENT ('Prestamistas', RESEED, 0);
-- Las tablas de catálogo (Estados, SistAmortizacion) suelen tener IDs fijos o Identity, reseteamos por si acaso
...
-- Insertar Estados Prestamo
SET IDENTITY_INSERT Estados_Prestamos ON;
INSERT INTO Estados_Prestamos (idEstado, descripcion) VALUES 
(1, 'Activo'),
(2, 'Finalizado'),
(3, 'Eliminado'),
(4, 'Archivado');
...
-- Insertar Estados Cuota
SET IDENTITY_INSERT Estados_Cuotas ON;
INSERT INTO Estados_Cuotas (idEstado, descripcion) VALUES 
(1, 'Pendiente'),
(2, 'Saldada'),
(3, 'Vencida'),
(4, 'Reprogramada');
...
-- Insertar Sistemas de Amortización
SET IDENTITY_INSERT SistAmortizacion ON;
INSERT INTO SistAmortizacion (idSistAmortizacion, descripcion) VALUES 
(1, 'Francés'),
(2, 'Alemán'),
(3, 'Americano'),
(4, 'Tasa Fija');
...
INSERT INTO MediosDePago (idMedio_Pago, descripcion, moneda) VALUES 
(1, 'Efectivo', 'ARS'),
(2, 'Transferencia', 'ARS'),
(3, 'Depósito', 'ARS');
SET IDENTITY_INSERT MediosDePago OFF;

-- Insertar Prestamista (Usuario Admin)
-- Nota: El hash de contraseña es un ejemplo. Si no funciona, registrar un nuevo usuario desde la app.
-- Hash para 'Password123!' (Ejemplo genérico de Identity V3)
INSERT INTO Prestamistas (usuario, contraseniaHash, nombre, apellido, correo, esActivo) VALUES 
('admin', 'AQAAAAEAACcQAAAAEH8J...', 'Admin', 'Sistema', 'admin@tucredito.com', 1);

-- Insertar Garantes
INSERT INTO Garantes (nombre, apellido, telefono, domicilio, correo, esActivo) VALUES 
('Carlos', 'Gomez', '1122334455', 'Av. Siempre Viva 123', 'carlos.gomez@email.com', 1),
('Ana', 'Martinez', '1144556677', 'Calle Falsa 456', 'ana.martinez@email.com', 1);

-- Insertar Prestatarios
-- Asumiendo DNI como PK manual
INSERT INTO Prestatarios (DNI, nombre, apellido, telefono, domicilio, correo, esActivo, idGarante) VALUES 
(12345678, 'Juan', 'Perez', '1155667788', 'San Martin 100', 'juan.perez@email.com', 1, 1),
(87654321, 'Maria', 'Lopez', '1166778899', 'Belgrano 200', 'maria.lopez@email.com', 1, 2);

-- Insertar Prestamos
-- Prestamo 1: Activo, Sistema Francés
...
(12345678, 1, 100000.00, 5.00, 12, GETDATE(), DATEADD(MONTH, 1, GETDATE()), DATEADD(MONTH, 12, GETDATE()), 1, 1, 95000.00);

-- Prestamo 2: Activo, Tasa Fija
INSERT INTO Prestamos (DNI_Prestatario, idPrestamista, montoOtorgado, tasaInteres, Cantidad_ctas, fechaOtorgamiento, Fec_1erVto, fechaFinEstimada, idEstado, idSistAmortizacion, saldoRestante) VALUES 
(87654321, 1, 50000.00, 10.00, 6, GETDATE(), DATEADD(MONTH, 1, GETDATE()), DATEADD(MONTH, 6, GETDATE()), 1, 4, 45000.00);

-- Insertar Cuotas (Ejemplo para Prestamo 1)
-- Se insertan solo algunas cuotas de prueba
INSERT INTO Cuotas (idPrestamo, nroCuota, Fec_Vto, Monto, Interes, SaldoPendiente, idEstado) VALUES 
(1, 1, DATEADD(MONTH, 1, GETDATE()), 10000.00, 500.00, 10000.00, 1), -- Pendiente
(1, 2, DATEADD(MONTH, 2, GETDATE()), 10000.00, 480.00, 10000.00, 1), -- Pendiente
(1, 3, DATEADD(MONTH, 3, GETDATE()), 10000.00, 460.00, 10000.00, 1); -- Pendiente

-- Insertar Cuotas (Ejemplo para Prestamo 2)
INSERT INTO Cuotas (idPrestamo, nroCuota, Fec_Vto, Monto, Interes, SaldoPendiente, idEstado) VALUES 
(2, 1, DATEADD(MONTH, 1, GETDATE()), 9000.00, 500.00, 9000.00, 1),
(2, 2, DATEADD(MONTH, 2, GETDATE()), 9000.00, 500.00, 9000.00, 1);

-- Insertar Pagos (Ejemplo de pago parcial en Prestamo 1, Cuota 1)
-- Nota: Esto no actualiza el estado de la cuota automáticamente, es solo dato histórico
INSERT INTO Pagos (idCuota, monto, Fec_Pago, idMedioPago, estado, observaciones, saldo) VALUES 
(1, 5000.00, GETDATE(), 1, 'Aprobado', 'Pago a cuenta', 5000.00);
